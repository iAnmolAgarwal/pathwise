...
describe("computeGap", () => {
  it("returns only skills below target level, tagged goal", () => {
    const gap = computeGap(profileWith({ ts: 2, js: 2, basics: 1 }), goals[0].requiredSkills, skills);
  ...
  it("is level-aware: a partially known skill is still a gap", () => {
    const gap = computeGap(profileWith({ react: 1, js: 2, basics: 1, ts: 2 }), goals[0].requiredSkills, skills);
  ...
  it("expands transitive prerequisites and tags them prereq-of", () => {
    const gap = computeGap(profileWith({}), goals[0].requiredSkills, skills);
  ...
  it("does not add prerequisites the learner already knows", () => {
    const gap = computeGap(profileWith({ js: 2, basics: 1 }), goals[0].requiredSkills, skills);
    expect(gap.map((g) => g.skillId).sort()).toEqual(["react", "ts"]);
  });

  it("marks a skill 'goal' even if it is also someone's prerequisite", () => {
    const gap = computeGap(profileWith({}, ["next-dev"]), goals[1].requiredSkills, skills);
  ...
  it("records graphPath for a prerequisite through to the goal it serves", () => {
    const gap = computeGap(profileWith({ basics: 1 }), goals[0].requiredSkills, skills);
  ...
  it("returns an empty gap when everything is known", () => {
    expect(computeGap(profileWith({ react: 3, ts: 2, js: 2, basics: 1 }), goals[0].requiredSkills, skills)).toEqual([]);
  });
});
