...
what production uses, but any local Postgres works — `DATABASE_URL=postgresql://postgres:postgres@localhost:5432/pathwise`
is what CI uses); an Anthropic API key; a Google OAuth client (see "Sign-in" — ten
minutes, no review). Python is only needed to re-run the offline pipeline.

1. `git clone https://github.com/iAnmolAgarwal/pathwise.git && cd pathwise` — or, from the
   source zip, `unzip pathwise-source.zip && cd pathwise`.
2. `npm install`
3. `cp .env.example .env.local` and fill in the values (table below). Leave the commented
   lines commented: an empty `AUTH_URL=` is not "unset" to Auth.js and turns host trust off,
   which breaks sign-in.
...
7. Sign in once at http://localhost:3000, then `npm run seed you@example.com` loads the
   ...
   the copies. The seed only needs that email to exist in the `users` table, so a row
   inserted by hand works when a Google client is not set up yet.

To run the production build locally (`npm run build && npm start`), Auth.js needs to be told
the host is yours: uncomment `AUTH_TRUST_HOST=true` in `.env.local` for that run (Vercel sets
it for you in production).
...
| `AUTH_URL` | Production only: the deployed origin, e.g. `https://pathwise.example.app`. Locally keep the line commented out, not blank — Auth.js treats an empty value as set and stops trusting the host |
| `AUTH_TRUST_HOST` | Only for `npm start` on localhost (uncomment, `true`); Vercel provides it in production |
...
| `npm run lint` | ESLint |
| `npm run test:e2e` | Playwright smoke test: the whole degraded journey against a dev server with an invalid model key (needs `DATABASE_URL` and a one-time `npx playwright install chromium`; creates and removes its own rows). The first run on a cold checkout also compiles the app, so it is the slow one |
